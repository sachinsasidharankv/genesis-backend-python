...
import requests
from src.voice.config import DEEPGRAM_API_KEY
...
async def deepgram_tts_stream(text: str):
    ...
    with requests.post(
        DEEPGRAM_TTS_ENDPOINT, stream=True, headers=headers, json=payload
    ) as r:
...
async def stream_audio_to_websocket(websocket: WebSocket, text: str):
